...
class TestAuthenticationEndpoints(TestConfiguration):
    ...
    def test_protected_endpoint_with_auth(self, test_client, auth_headers, mock_document_service):
...
class TestDocumentUploadEndpoints(TestConfiguration):
    ...
    def test_document_upload_success(self, test_client, auth_headers, mock_document_service):
    ...
    def test_document_upload_invalid_file_type(self, test_client, auth_headers):
        ...
        response = test_client.post(
            ...
            headers=auth_headers,
            files={
                "file": ("test_document.exe", test_file_content, "application/octet-stream")
            }
        )
...
class TestDocumentStatusEndpoints(TestConfiguration):
    ...
    def test_get_document_status_success(
        ...
        mock_cache_service.get_document_status = AsyncMock(return_value=mock_status_data)
        
        response = test_client.get(
            "/api/documents/test-doc-123/status",
            headers=auth_headers
        )
    ...
    def test_get_document_status_not_found(
        ...
        response = test_client.get(
            "/api/documents/nonexistent-doc/status",
            headers=auth_headers
        )
...
class TestDocumentListEndpoints(TestConfiguration):
    ...
    def test_list_user_documents(self, test_client, auth_headers, mock_document_service):
        ...
        mock_document_service.get_user_documents = AsyncMock(return_value=mock_documents)
    ...
    def test_list_user_documents_with_pagination(
        ...
        response = test_client.get(
            "/api/documents?limit=5&offset=10",
            headers=auth_headers
        )
