...
class DocumentIntelligenceService:
    ...
    async def process_document(
        ...
        try:
            ...
            if not enhanced_result.get("success"):
                logger.warning(f"LLM enhancement failed for {document_id}, using raw data")
    ...
    async def _update_processing_statistics(
        ...
        if success:
            ...
            current_successes = (self.processing_stats["success_rate"] / 100.0) * (total_docs - 1) + 1
            self.processing_stats["success_rate"] = (current_successes / total_docs) * 100
        else:
            # Recalculate success rate
            total_docs = self.processing_stats["documents_processed"]
            current_successes = (self.processing_stats["success_rate"] / 100.0) * (total_docs - 1)
            self.processing_stats["success_rate"] = (current_successes / total_docs) * 100 if total_docs > 0 else 0
