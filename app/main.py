...
from core.database_manager import get_database_session, initialize_database_connection
from services.document_intelligence_service import DocumentIntelligenceService
from services.cache_service import CacheService
from core.authentication import create_access_token, get_authenticated_user
...
async def application_lifespan(app: FastAPI):
    ...
    logger.info(f"Starting Document Intelligence System v{application_settings.application_version}")
...
def create_application() -> FastAPI:
    ...
    if application_settings.is_production:
        app.add_middleware(
            ...
            allowed_hosts=application_settings.allowed_origins
        )
...
async def health_check(request):
    ...
    services_status = {
        "database": "healthy",
        "cache": "healthy" if cache_service and await cache_service.health_check() else "unhealthy",
        "document_intelligence": "healthy" if document_service and await document_service.health_check() else "unhealthy"
    }
...
async def upload_document(
    ...
    # Save file
    file_path = Path(application_settings.upload_directory) / f"{document_id}_{file.filename}"
...
if __name__ == "__main__":
    ...
    uvicorn.run(
        ...
        workers=1 if application_settings.is_development else application_settings.worker_count
    )
