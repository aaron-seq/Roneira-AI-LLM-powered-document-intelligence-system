...
class WebSocketConnectionManager:
    ...
    async def broadcast_to_document(
        ...
        logger.info(f"Broadcast to document {document_id}: {successful_sends} successful, "
    ...
    def get_document_list(self) -> list[str]:
        ...
        """
        return [doc_id for doc_id, connections in self._connections.items() 
                ...
                for websocket in self._connections[document_id]:
                    if websocket in self._connection_metadata:
                        ...
                        metadata["client_info"] = {
                            ...
                            "has_query_params": len(metadata["client_info"]["query_params"]) > 0
                        }
                        stats["connection_details"][document_id]["connection_metadata"].append(metadata)
