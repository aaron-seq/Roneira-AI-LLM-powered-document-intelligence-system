...
class TimedContext:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb) -> None:
        ...
        if exc_type is not None:
            ...
            self._logger.info(
                f"Completed {self._operation}", duration_ms=round(duration_ms, 2)
            )


def setup_structured_logging(
    service_name: str = "roneira", log_level: str = "INFO", json_output: bool = True
) -> None:
    ...
    async def __call__(self, scope, receive, send):
        ...
        if scope["type"] == "http":
            ...
            correlation_id = headers.get(
                b"x-correlation-id", headers.get(b"x-request-id", b"")
