...
class TracingService:
    ...
    def _init_tracer(self, enable_console: bool) -> None:
        ...
        resource = Resource.create(
            {"service.name": self.service_name, "service.version": "2.0.0"}
        )
...
def trace_llm_operation(
    provider: str, model: str, operation: str = "generate"
) -> Callable:
            ...
            with tracer.create_span(span_name, attrs) as span:
                try:
                    ...
                    if hasattr(result, "usage") and span:
                        span.set_attribute(
                            "llm.tokens.total", result.usage.get("total_tokens", 0)
                        )
...
def init_tracing(
    service_name: str = "roneira-document-intelligence", enable_console: bool = False
) -> TracingService:
    ...
    global _tracing_service
    _tracing_service = TracingService(
        service_name=service_name, enable_console_export=enable_console
    )
    return _tracing_service
