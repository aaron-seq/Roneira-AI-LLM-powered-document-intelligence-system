...
class FreeOCRService:
    ...
    def __init__(self):
        ...
        self.use_easyocr = (
            os.getenv("USE_EASYOCR", "true").lower() == "true" and EASYOCR_AVAILABLE
        )
        ...
        logger.info(
            f"OCR Service initialized. Tesseract: ✓, EasyOCR: {'✓' if self.use_easyocr else '✗'}"
        )
    ...
    async def extract_text_from_pdf(self, pdf_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            for page_num in range(len(doc)):
                ...
                text = page.get_text()
                pages_text.append(
                    {"page": page_num + 1, "text": text, "word_count": len(text.split())}
                )
        ...
        try:
            ...
            if self.use_easyocr:
                try:
                    ...
                    easyocr_confidence = (
                        np.mean(easyocr_confidences) if easyocr_confidences else 0
                    )
            ...
            best_confidence = (
                results["tesseract"]["confidence"] if results["tesseract"] else 0
            )
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        health = {"tesseract": False, "easyocr": False, "services_available": []}
