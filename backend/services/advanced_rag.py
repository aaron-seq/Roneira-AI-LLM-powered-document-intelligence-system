...
class CrossEncoderReranker:
    ...
    async def rerank(
        self, query: str, chunks: List[RetrievedChunk], top_k: Optional[int] = None
    ) -> List[RetrievedChunk]:
...
class ContextCompressor:
    ...
    def __init__(self, max_tokens: int = 2048, llm_provider=None):
    ...
    def compress_extractive(
        self, chunks: List[RetrievedChunk], query: str, char_limit: int = 8000
    ) -> str:
    ...
    async def compress_abstractive(self, chunks: List[RetrievedChunk], query: str) -> str:
...
class AdvancedRAGPipeline:
    ...
    def __init__(
        ...
        self.compressor = (
            ContextCompressor(llm_provider=llm_provider) if enable_compression else None
        )
    ...
    async def retrieve(
        ...
        for expanded_query in context.expanded_queries:
            chunks = await self._retrieve_for_query(
                expanded_query, self.initial_retrieval_k, filter_dict
            )
        ...
        if self.reranker and all_chunks:
            all_chunks = await self.reranker.rerank(query, all_chunks, self.final_k * 2)
    ...
    async def generate(
        ...
        response = await self._llm.generate(
            prompt, system_prompt=system_prompt or default_system
        )

        return response.content

    async def _retrieve_for_query(
        self, query: str, top_k: int, filter_dict: Optional[Dict[str, Any]]
    ) -> List[RetrievedChunk]:
...
async def create_configured_pipeline(
    vector_store=None, embedding_service=None, llm_provider=None
) -> AdvancedRAGPipeline:
