...
class CrossReferenceService:
    ...
    def find_entity_links(
        self, document_id: str, min_shared: int = 2
    ...
    def find_topic_links(
        self, document_id: str, min_shared: int = 1
    ) -> List[DocumentReference]:
        ...
        for other_id, other_topics in self._doc_topics.items():
            ...
            if len(shared) >= min_shared:
                overlap_ratio = len(shared) / min(len(source_topics), len(other_set))
    ...
    def cluster_documents(self, n_clusters: int = 5) -> List[DocumentCluster]:
    ...
    def _simple_kmeans(
        self, embeddings: List[List[float]], k: int, max_iter: int = 100
    ) -> List[List[int]]:
