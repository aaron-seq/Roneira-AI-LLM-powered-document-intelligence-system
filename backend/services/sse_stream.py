...
class SSEStreamManager:
    ...
    def __init__(self, heartbeat_interval: float = 15.0, max_reconnect_time: int = 30000):
    ...
    async def stream_llm_response(
        ...
        try:
            ...
            yield SSEEvent(
                data={"error": str(e), "status": "error"}, event="error"
            ).encode()
        finally:
    ...
    async def create_streaming_response(
        ...
        async def event_generator():
            async for event in self.stream_llm_response(response_generator, stream_id):
...
async def stream_chat_response(
    ...
    return await manager.create_streaming_response(
        chunk_generator(), stream_id=session_id, request=request
    )
