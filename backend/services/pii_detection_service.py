    ...
    def __init__(self):
        ...
        self._patterns = {
            ...
            PIIType.DATE_OF_BIRTH: [
                (
                    r"\b(?:dob|date\s+of\s+birth|birthdate)[:\s]+\d{1,2}[-/]\d{1,2}[-/]\d{2,4}\b",
                    0.95,
                ),
            ...
            PIIType.ADDRESS: [
                (
                    r"\b\d+\s+[A-Za-z]+(?:\s+[A-Za-z]+)*\s+(?:Street|St|Avenue|Ave|Road|Rd|Boulevard|Blvd|Lane|Ln|Drive|Dr|Court|Ct|Way|Place|Pl)\.?\b",
                    0.80,
                ),
            ],
        }
...
class PIIRedactor:
    ...
    def redact(self, text: str, matches: List[PIIMatch]) -> Tuple[str, List[PIIMatch]]:
    ...
    def _create_redaction(self, match: PIIMatch) -> str:
        ...
        method = self.config.per_type_methods.get(match.pii_type, self.config.method)
...
class PIIDetectionService:
    ...
    def detect(
        self, text: str, enabled_types: Optional[Set[PIIType]] = None
    ) -> List[PIIMatch]:
        """Detect PII in text.
        ...
        return self.detector.detect(
            text, enabled_types or self.config.enabled_types, self.config.custom_patterns
        )
    ...
    def generate_report(self, matches: List[PIIMatch]) -> Dict[str, Any]:
