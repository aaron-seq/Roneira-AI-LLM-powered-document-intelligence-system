...
class LLMProviderFactory:
    ...
    def create_provider(
        self, config: LLMConfig, instance_id: Optional[str] = None
    ) -> BaseLLMProvider:
        ...
        if provider_type not in PROVIDER_REGISTRY:
            raise LLMProviderError(
                f"Unsupported provider type: {provider_type}", provider="factory"
            )
    ...
    async def get_available_provider(self) -> Optional[BaseLLMProvider]:
        ...
        for config in self._fallback_chain:
            try:
                ...
                logger.warning(f"Provider {config.provider_type} unavailable: {e}")
                continue
