...
class AzureOpenAIProvider(BaseLLMProvider):
    ...
    async def initialize(self) -> None:
        ...
        if not AZURE_OPENAI_AVAILABLE:
            raise LLMProviderError("openai package not installed", self.provider_name)

        if not self.config.azure_api_key or not self.config.azure_endpoint:
            raise LLMProviderError(
                "Azure OpenAI API key and endpoint are required", self.provider_name
            )
    ...
    async def generate_stream(
        ...
        try:
            ...
            raise LLMProviderError(
                f"Azure OpenAI stream error: {e}", self.provider_name, original_error=e
            )
