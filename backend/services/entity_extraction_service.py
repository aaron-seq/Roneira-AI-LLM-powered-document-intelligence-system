...
class RelationshipExtractor:
    ...
    def __init__(self):
        ...
        self._relation_patterns = [
            (
                r"(\w+)\s+(?:is|was|are|were)\s+(?:a|an|the)?\s*(\w+)\s+(?:of|for|at)\s+(\w+)",
                "is_X_of",
            ),
            ...
            (
                r"(\w+)\s+(?:is|was)\s+(?:located|based|headquartered)\s+(?:in|at)\s+(\w+)",
                "located_in",
            ),
        ]
    ...
    def extract_relationships(
        ...
        for sent in sentences:
            sent_entities = [e for e in entities if e.text.lower() in sent.lower()]
    ...
    def _detect_relation(
        self, context: str, entity1: Entity, entity2: Entity
    ) -> Optional[str]:
        ...
        if (
            entity1.entity_type in (EntityType.PERSON, EntityType.ORGANIZATION)
...
class EntityExtractionService:
    ...
    async def extract_entities(
        self, text: str, include_patterns: bool = True, include_ner: bool = True
    ) -> List[Entity]:
