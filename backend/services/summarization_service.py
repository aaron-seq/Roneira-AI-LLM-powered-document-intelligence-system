...
class DocumentSummarizer:
    ...
    async def summarize(
        self, text: str, config: Optional[SummaryConfig] = None
    ) -> SummaryResult:
        ...
        try:
            ...
            return SummaryResult(
                ...
                compression_ratio=len(truncated.split()) / original_words
                if original_words
                else 0,
            )

    def _extractive_summarize(self, text: str, config: SummaryConfig) -> str:
    ...
    async def _hybrid_summarize(self, text: str, config: SummaryConfig) -> str:
    ...
    def _extract_key_phrases(self, text: str, top_k: int = 5) -> List[str]:
        ...
        filtered = [
            ...
            if phrase.split()[0] not in common_starts and count > 1
        ]

        return filtered[:top_k]

    async def summarize_multiple(
        self, documents: List[Dict[str, str]], config: Optional[SummaryConfig] = None
    ) -> List[SummaryResult]:
        ...
        """
        tasks = [self.summarize(doc["text"], config) for doc in documents]
